...
\vspace{1em}
\begin{lstlisting}[caption={LOB Matching Algorithm}, label={lst:match_order}]
...
Our testing procedures were critical in identifying flaws that were not initially apparent. A significant logic error was discovered regarding an LOB invariant violation. During early testing, the Matching Engine incorrectly attempted to match against the worst ask price rather than the best ask price, as the engine was accessing the end of the ascending price list instead of the beginning. While simple unit tests missed this edge case, running a randomized order submission simulation revealed that ``Ask'' prices were crossing below ``Bid'' prices—a clear violation of market invariants.
