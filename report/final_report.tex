...
We used Google Benchmark across four branches. The baseline is the benchmark branch, built in Release mode with assertions and contract checks disabled. The lob-dsa, mem-pool, and template branches are exploratory prototypes for potential future improvements, rather than immediate production defaults.

\noindent\textbf{Matching Engine}

Figure \ref{fig:branch_benchmark_10k} compares the 10,000-order workload across the main latency-sensitive operations. The memory-pool branch is the only variant that consistently improves end-to-end performance over the baseline. Its strongest gain is in no-match order insertion (\texttt{BM\_MatchingEngine\_NewOrderNoMatchBurstLatency}), where latency is reduced from 0.807 ms to 0.434 ms ($\sim$1.86$\times$ faster). It also reduces pure LOB insertion latency from 0.672 ms to 0.257 ms ($\sim$2.61$\times$ faster). In contrast, the template-specialization branch is mostly neutral to slightly slower, and the lob-dsa branch is effectively neutral for these engine-level benchmarks.
...
For the ring buffer component, we measured average per-event latency across different input amounts (events per burst). The measured averages are 27.37 ns (64 events), 11.61 ns (512 events), and 10.71 ns (4096 events), as shown in Figure \ref{fig:ring_latency_input}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{assets/ring_buffer_latency_by_input.png}
    \caption{MPSC ring buffer average latency by input amount.}
    \label{fig:ring_latency_input}
\end{figure}

\noindent\textbf{WebSocket}

For the websocket benchmark on the baseline branch, \texttt{BM\_Websocket\_ClientToServerRoundTripLatency} measures mean round-trip latency at three payload sizes:
\begin{itemize}
    \item 64 B: 4.045 ms (\(\sim\)247 round-trips/s)
    \item 512 B: 4.066 ms (\(\sim\)246 round-trips/s)
    \item 4096 B: 4.132 ms (\(\sim\)242 round-trips/s)
\end{itemize}
This indicates near-constant websocket round-trip cost over small-to-moderate payload sizes in our local setup.
