...
\begin{itemize}
    ...
    \end{equation}
    where $\mu$ is the drift, $\sigma$ is the volatility, $W_t$ is a standard Wiener process, and $J_t$ represents the jump process, modeled as a compound Poisson process with a specified jump intensity $\lambda$ and a random jump size distribution. This jump term explicitly allows the model to simulate sudden, discontinuous price changes driven by exogenous shocks, such as breaking macroeconomic news or earnings reports. The Informed Trader continuously polls the order book and the oracle. If it detects a mispricing greater than a threshold $\epsilon$ (e.g., $V_t > P_{ask} + \epsilon$), it executes aggressive snipe orders against lagging market maker quotes. This introduces adverse selection and efficiently drives the simulated market towards price discovery.
\end{itemize}
