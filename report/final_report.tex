...
For the ring buffer component, we measured single-event producer-consumer handoff latency. On the benchmark-branch rerun, we observed 771 ns in a single run, with aggregate mean 648.76 ns (manual-time) and 673 ns (CPU-time). This benchmark currently has one configured input amount (one event handoff), shown in Figure \ref{fig:ring_latency_input}.
